...
RTC_DATA_ATTR int MyGSRLineWatchFaceId; // Watchface ID #.
// Approx. 3035 bytes

class GSRWatchFaceLine : public WatchyGSR
{
public:
  GSRWatchFaceLine() : WatchyGSR() { initAddOn(this); } // *** DO NOT EDIT THE CONTENTS OF THIS FUNCTION ***

  LineWatchy LineWatchFace;

  void RegisterWatchFaces()
  { // Add WatchStyles here (this is done pre-boot, post init)
    MyGSRLineWatchFaceId = AddWatchStyle("Line", this);
  };

  // void InsertInitWatchStyle(uint8_t StyleID){
  //   if (StyleID == GSRLineWatchFaceId){ // LCD
  //       Design.Menu.Top = 56;
  ...
  void InsertDrawWatchStyle(uint8_t StyleID)
  {
    uint8_t X, Y;
    uint16_t A;
    if (StyleID == MyGSRLineWatchFaceId)
    {
      if (SafeToDraw())
      {
        LineWatchFace.drawWatchFace();
      }
    }
  };
};

GSRWatchFaceLine MyGSRWatchFaceLine;
